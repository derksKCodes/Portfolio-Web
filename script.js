  ...
  fetch("http://localhost:8080/get_testimonials")
    ...
    .then((data) => {
      ...
      data.forEach((testimonial) => {
        ...
        const stars = "⭐".repeat(testimonial.rating); // Create star rating string
        ...
        newTestimonial.innerHTML = `     
                ...
                <img src="${profileImage}" alt="Client Profile" class="profile-pic">
                    ...
                    <p>"${testimonial.feedback}"</p>
                        ...
                        <p class="author">- ${testimonial.name}, ${testimonial.position}</p>                    
                        <p class="rating" id="rating_stars">${stars}</p>     
...
document
  .getElementById("form_Data")
  .addEventListener("submit", function (event) {
    event.preventDefault(); // Stop the form from submitting normally

    // Collect form values
    let name = document.getElementById("name").value;
    let Cposition = document.getElementById("position").value; // New field for position
    let feedback = document.getElementById("feedback").value;
    let rating = document.getElementById("rating_stars").value;
    let profileInput = document.getElementById("profile");

    // Prepare form data for sending (including file)
    let formData = new FormData();
    formData.append("name", name);
    formData.append("position", Cposition); // Append position to form data
    ...
    fetch("http://localhost:8080/submit_testimonial", {
      ...
      .then((data) => {
        ...
        document.getElementById("rating_stars").value = "";
  ...
  .addEventListener("submit", function (e) {
    ...
    fetch("http://localhost:8080/submit_contact", {
      ...
      .catch((error) => {
        ...
        alert(
          "Something went wrong while sending your message. Error details: " +
            error.message
        );
      });
  });
